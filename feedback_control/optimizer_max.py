# -*- coding: utf-8 -*-
"""
Created on Tue Jul 28 13:16:42 2026
...
MIN_ALPHA = 1e1   # Lower bound for alpha
ALPHA_GROWTH = 1.5   # Multiplier when increasing alpha
ALPHA_SHRINK = 0.5 # Multiplier when decreasing alpha
...
def optimize_step_fd_adaptive(V, step, alpha, settle, set_voltage, measure_power, p_base=None):
    ...
    if gradient_norm * alpha > HIGH_THRESH:
        new_alpha = alpha *  ALPHA_SHRINK  # Large gradient smaller steps
    elif gradient_norm * alpha < LOW_THRESH:
        new_alpha = alpha * ALPHA_GROWTH  # Small gradient larger steps
    else:
        new_alpha = alpha * 1.05                 # Keep current alpha
    ...
    # for k in range(N):
    #     raw_grad = (P_plus - P_minus) / (2 * step)
        
    #     momentum_grad[k] = (BETA * momentum_grad[k]) + ((1-BETA)*raw_grad)
    
    for k in range(N):
        momentum_grad[k] = (BETA * momentum_grad[k]) + ((1-BETA)*grads[k])
    

    # Update voltages with new_alpha
    # Update voltages with new_alpha (Maximization / Gradient Ascent)
    for k in range(N):
        V[k] = _clip(V[k] - new_alpha * momentum_grad[k])
        set_voltage(k, V[k])
