...
def generate_waveforms(
    ...
    *,
    ...
    ROLL_OFF_C=0.1,
    LFSR_LENGTH=15,                 # Shift register length
    # ---- Quantum channel ----
    M=64,                            # Constellation size (e.g. 64 for 64-QAM)
    ...
    # Quantum signal: Gaussian-shaped N-QAM -> upsample -> RRC -> shift
    print(f"Quantum: {M}-QAM at {BAUD_Q/MHz:g} Mbaud, shifted to {FREQ_SHIFT_Q/GHz:g} GHz")
    np.random.seed(seed)   # Reproducible symbol draw for matched receiver
    ...
    metadata = {
        ...
        'pilot_tone_amplitudes': amplitudes,
        'data_amplitude':  A_DATA,
        'M':              M,
        'nu':             NU,
        'f_c':            BAUD_C,
...
def generate_quantum_only_waveform(metadata: dict) -> tuple:
    ...
    Parameters are pulled from `metadata` (as produced by generate_waveforms)
    so the quantum-only replay stays consistent with the original emission.
    Pass in a modified metadata dict to override any of them.
    """
    M            = metadata['M']
    NU           = metadata['nu']
...
if __name__ == '__main__':
    # Example: run with defaults (equivalent to the original hard-coded script)
    tx_shift_c, tx_shift_q, metadata = generate_waveforms(show_plots=True)

    # Example: override parameters when calling
